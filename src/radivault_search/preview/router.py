...
async def get_thumbnail(
    ...
    """Stream the 256x256 JPEG thumbnail for an ingested study (FR-API-1).

    Kyle 2026-04-27 — relaxed the legacy ``_verified_or_403`` gate. The
    thumbnail is generated by ``thumbnail.py`` ONLY when the source DICOM
    passed the BurnedInAnnotation gate AND the modality whitelist
    (CT/MR/CR/DR/DX/MG) at ingest. So `preview_thumbnail_key` being set
    is itself proof of PHI scrub. Studies whose ingest emitted no key
    (still pending re-ingest, or modality outside the whitelist) fall
    through to the 404 below — buyer-portal renders the empty state for
    those.
    """
    ...
    with factory() as session:
        ...
        # `preview_thumbnail_key` is the ingest-set PHI-safe-by-construction
        # signal. Fall back to the deterministic key only when the column
        # is not yet populated (legacy demo-seed studies pre-FR-INGEST-1).
        if study.preview_thumbnail_key is None and study.preview_status not in _VERIFIED_STATUSES:
            raise FrameNotFound(
                detail=f"study {study.pseudo_study_uid!r} has no thumbnail asset",
            )
